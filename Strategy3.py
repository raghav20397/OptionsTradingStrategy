...
def computeExpectedSpotChanges(year,month,day, data,windowSize,timeGap, type, smoothingFactor):
    ...
    buffer = []
    # Initialization of buffer
    while(iter < datetime(year,month,day,9,15,windowSize)) and j < len(SpotPrices):
        buffer.append(SpotPrices[j])
...
def plotValFit(windowSize,TimeGap,data,year,month,date,targetStrike):
    ...
    buffer = []
    # Initialization of buffer
    while(iter < datetime(year,month,date,9,15,windowSize+TimeGap) and j<len(SpotPrices)):
...
def ccomputePremiumExpectedDelta(actualValues, expectedValues, expectedSpotChanges, windowSize, timeGap, targetStrike,hourFrom, minuteFrom, secondFrom, hourTo, minuteTo, secondTo, date, month, year, estimation_type,type="now"):
    ...
    for i in range(len(actualValues)):
        if(i<timeGap):
            ...
            #   MX  +  C 
            if(type=="predict" and i<len(expectedSpotChanges)):
            # delta for t+5
...
def computePremiumExpectedVega(actualValues, expectedValues, expectedImpVChanges, windowSize, timeGap, targetStrike,hourFrom, minuteFrom, secondFrom, hourTo, minuteTo, secondTo, date, month, year, estimation_type,type="now"):
    ...
    for i in range(len(expectedImpVChanges)):
        if(i<timeGap):
            ...
            if(type=="now" and i<len(expectedImpVChanges) ):
                # try:
...
def computeExpectedImpvChanges(actualImpvs,windowSize,timeGap,year,month,day,type):
    ...
    buffer = []
    # Initialization of buffer
    while(iter < datetime(year,month,day,9,15,windowSize) and j<len(actualImpvs)):
    ...
    while time_iter < lenBefore + len(timeList)-timeGap:
        ...
        if time_iter+timeGap< len(expectedPremiums):
            ...
            # if( ((abs(expectedPremiums[time_iter+timeGap].premium - actualPremiums[time_iter].price) / actualPremiums[time_iter].price) * 100  >100*brockerage) and (0.7>=actualPremiums[time_iter + timeGap].delta >= 0.3 or -0.7<=actualPremiums[time_iter + timeGap].delta <= -0.3)):
                ...
                if expectedPremiums[time_iter+timeGap].premium-actualPremiums[time_iter].price >= 0 and actualPremiums[time_iter+timeGap].price  - actualPremiums[time_iter].price  >= 0 and actualPremiums[time_iter].delta >= -0.5 and actualPremiums[time_iter].delta <= 0.5 and actualPremiums[time_iter].price>=15:
                ...
                # expected pnl>0 ,  actual pnl<0, brock+
                elif expectedPremiums[time_iter+timeGap].premium-actualPremiums[time_iter].price  >= 0 and actualPremiums[time_iter+timeGap].price  - actualPremiums[time_iter].price  <= 0 and actualPremiums[time_iter].delta >= -0.5 and actualPremiums[time_iter].delta <= 0.5 and actualPremiums[time_iter].price>=15:
...
#         writer = csv.writer(f)
#         writer.writerows(rows)
def csvforyear(pathtocreate, greek, greek_use,dictPnL,dictHitRate,dictCap,dictProfPercent,year,estimationType,windowSize,timeGap,optionType):
    ...
    for smoothingFactor in dictPnL.keys():
        ...
        av_hr = 0
        av_prof = 0
        rows.append(['Month', 'PnL', 'Capital Required', 'Average Hit Rate', '% profit'])
        filename = f"{smoothingFactor}_{greek}_{estimationType}_{str(windowSize)}_{str(timeGap)}_{optionType}_{greek_use}.csv"
        for month in dictPnL[smoothingFactor].keys():
            row = []
            row.append(month)
            row.append(dictPnL[smoothingFactor][month])
            tot_pnl+=dictPnL[smoothingFactor][month]
            row.append(dictCap[smoothingFactor][month])
            tot_cap+=dictCap[smoothingFactor][month]
            row.append(dictHitRate[smoothingFactor][month])
            av_hr+=dictHitRate[smoothingFactor][month]
            row.append(dictProfPercent[smoothingFactor][month])
            av_prof+=dictProfPercent[smoothingFactor][month]
        ...
        with open(location+"/"+filename,"w",newline="") as f:
            writer = csv.writer(f)
            writer.writerows(rows)
    return tot_pnl,tot_cap,av_hr,av_prof



def allcsv(pathtocreate, greek, greek_use,dictPnL,dictHitRate,dictCap,dictProfPercent,year,estimationType,windowSize,timeGap,optionType):
    os.chdir(pathtocreate)
    location = pathtocreate
    for smoothingFactor in dictPnL.keys():
        rows = []
        tot_pnl = 0
        tot_cap = 0
        av_hr = 0
        av_prof = 0
        rows.append(['Year', 'PnL', 'Capital Required', 'Average Hit Rate', '% profit'])
        filename = f"{smoothingFactor}_{greek}_{estimationType}_{str(windowSize)}_{str(timeGap)}_{optionType}_{greek_use}.csv"
        for year in dictPnL[smoothingFactor].keys():
            row = []
            row.append(year)
            row.append(dictPnL[smoothingFactor][year])
            tot_pnl+=dictPnL[smoothingFactor][year]
            row.append(dictCap[smoothingFactor][year])
            tot_cap+=dictCap[smoothingFactor][year]
            row.append(dictHitRate[smoothingFactor][year])
            av_hr+=dictHitRate[smoothingFactor][year]
            # row.append(dictProfPercent[smoothingFactor][year])
            try:
                row.append((dictPnL[smoothingFactor][year] / dictCap[smoothingFactor][year])*100)
            except:
                row.append(0)
            try:
                av_prof+=dictProfPercent[smoothingFactor][year]
            except:
                print(dictProfPercent[smoothingFactor][year])
            rows.append(row)
        # numYears = len(dictPnL[smoothingFactor].keys())
        av_hr/=5
        av_prof/=5
        rows.append(['','','','',''])
        rows.append(['','Total PnL','Total Capital Required','Average Hit Rate','Average % profit'])
                ...
                for option_type in data[expiry_date][trade_time]:
                    if(option_type == optionType):
                        ...
                        if found>0:
                            ...
                            # if there was no contract available for that time with given 
                            # strike and type then we just duplicate and push the last found contract (assumption)
                            if len(windowBuffer) < windowSize and len(windowBuffer) != 0:
                                # print(windowBuffer, len(windowBuffer), expiry_date)   
                                windowBuffer.append(windowBuffer[len(windowBuffer)-1])  
                            elif len(windowBuffer) == windowSize and len(windowBuffer)!=0:
                                windowBuffer.append(windowBuffer[len(windowBuffer)-1])
                                windowBuffer.pop(0)
                            try:
                                dup = copy.deepcopy(actualValues[len(actualValues)-1])
...
if __name__ == "__main__":
    ...
    spotPath = "/media/emkds/0CF661CAF661B51C/op_strat_raghav_prakrit/BANKNIFTY_spot_seconds_till_2022_10_12.json"
    ...
    # pathtopkl = "C:/Users/User/Desktop/op_strat_raghav_prakrit/BANKNIFTY"
    # pathtopkl = "X:/NXBLOCK/OptionsTradingStrategy/BANKNIFTY" # add the path
    pathtopkl = "/media/emkds/0CF661CAF661B51C/op_strat_raghav_prakrit/BANKNIFTY"  # add the path
    # pathtocreate = "D:/College Documents/ExtramarksOptionsTrading/OptionsTradingStrategy/Reports" # add the path
    # pathtocreate = "C:/Users/User/Desktop/op_strat_raghav_prakrit/AllReports"
    # pathtocreate = "X:/NXBLOCK/OptionsTradingStrategy/AllReports" # add the path
    pathtocreate = "/media/emkds/0CF661CAF661B51C/op_strat_raghav_prakrit/AllReports"  # add the path
    ...
    greek_use_i = "now"
    estimation_type_i = "ema"
    prev_windowSize = 3
    timeGap = 2

    # grk = "vega"
    # grk = "delta"
    for grk in ['delta','vega']:
        ...
        for year in os.listdir():
            # print(os.getcwd(),pathtopkl,year)
            if(year[-3:]=="csv"):
                continue
            ...
            for sf in smoothingFactors:
                year_pnl[sf] = dict()
                year_hitrates[sf] = dict()
                year_cap[sf] = dict()
                year_prof_per[sf] = dict()
            for month in tqdm(os.listdir()):
                ...
                month_pnl,month_cap,month_avhitr,prof_percent = csvformonth(pathtocreate,grk,greek_use_i, month_pnl,hitrates_fac,year,monthtonum[month],e_type_report,w_size_report,t_gap_report,op_type_report)
                # year_pnl[sf][month] = 0
                year_pnl[sf][month] = month_pnl
                # year_hitrates[sf][month] = 0
                year_hitrates[sf][month] = month_avhitr
                # year_cap[sf][month] = 0
                year_cap[sf][month] = month_cap
                # year_prof_per[sf][month] = 0
                year_prof_per[sf][month] = prof_percent

            all_pn,all_ca,all_avhtr,all_profperc = csvforyear(pathtocreate,grk,greek_use_i,year_pnl,year_hitrates,year_cap,year_prof_per,year,e_type_report,w_size_report,t_gap_report,op_type_report)
            # print(os.getcwd(), pathtocreate, pathtopkl)
            os.chdir(pathtopkl)
            all_pnl[sf][year]=all_pn
            all_cap[sf][year]=all_ca
            all_profper[sf][year] = all_profperc
            all_hitr[sf][year] = all_avhtr
        allcsv(pathtocreate,grk,greek_use_i,all_pnl,all_hitr,all_cap,all_profper,year,estimation_type_i,w_size_report,t_gap_report,op_type_report)
