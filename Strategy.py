...
# def computeGreeks(path, fileName, spotData, windowSize, timeGap, targetStrike, optionType, smoothingFactor,date=1, month=12, year=2020, hourFrom=9, minuteFrom=15, secondFrom=00, hourTo=15, minuteTo=30, secondTo=00, estimation_type="simple", greek_use="predict"):
#     # loading the pickle data
#     pkl_file_location = os.path.join(path, fileName)
#     data = load_pkl(pkl_file_location)   
...
# def PredChangeinImpliedVol(buffer,timeGap,windowSize,bFactor):
#     imp_vs = []
#     for i in range(len(buffer)):
#         imp_vs.append(buffer[i].imp_v)
    
#     predicted_imp_v = EMA(imp_vs,windowSize,bFactor,timeGap)

#     return predicted_imp_v

# def compPremExpectedImpV(contractBuffer,windowSize,timeGap,bFactor):
#     imp_vs = []
#     for i in range(windowSize):
#         imp_vs.append(contractBuffer[i].imp_v)
...
def computePremiumExpected(actualValues, expectedValues, expectedSpotChanges, windowSize, timeGap, targetStrike,hourFrom, minuteFrom, secondFrom, hourTo, minuteTo, secondTo, date, month, year, estimation_type,type="now"):
    ...
    # print(actualValues[-6].price, expectedValues[-1].delta, spotsExpected[-1].spot, expectedPremiums[-1].premium)
    # plotPremiumError(expectedPremiums,actualValues, timeList, targetStrike, windowSize, timeGap, year, month, date, lenBefore, estimation_type, type)
    # plotPremiumActual(expectedPremiums,actualValues, timeList, targetStrike, windowSize, expectedSpots, actualPremiums, expectedValues,timeGap, windowSize, optionType ,brockerage, targetStrike, date, month, year, hourFrom, minuteFrom, secondFrom, hourTo, minuteTo, secondTo, greek_type, estimation_type, smoothingFactor)
    ...
    if year not in os.listdir():
        os.mkdir(pathtocreate+"\\"+year)
    os.chdir(pathtocreate+"\\"+year)
    if month not in os.listdir():
        os.mkdir(pathtocreate+"\\"+year+"\\"+month)
    os.chdir(pathtocreate+"\\"+year+"\\"+month)
    if date not in os.listdir():
        os.mkdir(pathtocreate+"\\"+year+"\\"+month+"\\"+date)
    if "Reports" not in os.listdir():
        os.mkdir(pathtocreate+"\\"+year+"\\"+month+"\\"+date+"\\Reports")
    os.chdir(pathtocreate+"\\"+year+"\\"+month+"\\"+date+"\\Reports")
    location = pathtocreate+"\\"+year+"\\"+month+"\\"+date 
    time_iter = lenBefore + timeGap
    end_iter = lenAfter
    pnl = 0
    rows= []
...
def getStrikePrices(path,optionType,year,month,day):
    startDate = datetime.datetime(year,month,day,9,15,0)
    data = load_pkl(path)
    strikePrices = []
    for expiry_date in data:
        for strike_price in data[expiry_date][startDate][optionType]:
            strikePrices.append(strike_price)
    
    return strikePrices


def computeExpectedImpvChanges(actualImpvs,windowSize,timeGap,year,month,day):
    # SpotPrices = computeSpotsActual(year,month,day,data)
...
def computeGreeks(pathoriginal,pathtocreate,fileName, spotData, windowSize, timeGap, targetStrike, optionType, smoothingFactor,date=1, month=12, year=2020, hourFrom=9, minuteFrom=15, secondFrom=00, hourTo=15, minuteTo=30, secondTo=00, estimation_type="simple", greek_use="predict"):
    # loading the pickle data
    # pkl_file_location = os.path.join(path, fileName)
    ...
    for expiry_date in data:
        ...
        pnl = ProfitorLossforaDay(pathtocreate,expectedPremiums, spotsActual, expectedSpotsChanges, actualValues, expectedValues,timeGap, windowSize, optionType,0.01, targetStrike, date, month, year, hourFrom, minuteFrom, secondFrom, hourTo, minuteTo, secondTo, greek_use, estimation_type, smoothingFactor)
        ...
        if date not in os.listdir():
            os.mkdir(pathtocreate+"\\"+year+"\\"+month+"\\"+date)
        location1 = pathtocreate+"\\"+year+"\\"+month+"\\"+date 
        file = open(location1, "a")
        file.write(str(strikePnl[0])+" " +f"{windowSize}_{timeGap}" + f"{optionType}" +f"{smoothingFactor}"+"---->" + str(strikePnl[1]) +"\n" )
        file.close()
...
if __name__ == "__main__":
    ...
    spotData = load_json(spotPath)
    print("Spot JSON loaded ")
    # pathtopkl = "D:\\Desktop\\College Documents\\ProjectExtramarks2\\OptionsTradingStrategy" # add the path
    pathtopkl = "D:\\College Documents\\ExtramarksOptionsTrading\\OptionsTradingStrategy\\BANKNIFTY"
    pathtocreate = "D:\\College Documents\\ExtramarksOptionsTrading\\OptionsTradingStrategy\\Reports"
    # pathtocreate = "D:\\Desktop\\College Documents\\ProjectExtramarks2\\OptionsTradingStrategy" # add the path
                                ...
                                if(estimation_type_i == "ema"):
                                    for smoothingFactor in [0.01, 0.03, 0.3, 0.4]:
                                        for targetStrike in strikePrices:
                                            computeGreeks(pathtopkl+"\\"+year+"\\"+month+"\\"+date,pathtocreate,date, spotData, prev_windowSize, timeGap, targetStrike, optionType, smoothingFactor,dateNum, monthNum, yearNum, hourFrom=9, minuteFrom=15, secondFrom=0, hourTo=15, minuteTo=30, secondTo=0, estimation_type=estimation_type_i, greek_use=greek_use_i)
                                else:
